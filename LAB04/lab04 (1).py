...
def check_threats(board, player):
    ...
    for i, j in get_empty_cells(board):
        ...
        if is_winner(board, player):
            threat_move = (i, j)
        board[i][j] = ' '  # Reset the board to its original state

    if 'threat_move' in locals():
        return threat_move
    else:
        return None
...
def find_best_move(board):
    # Check for immediate threats from the opponent and block them
    threat_move = check_threats(board, 'X')
    if threat_move:
        return threat_move

    # If no immediate threat, use minimax to find the best move
    best_val = float('-inf')
    best_move = (-1, -1)
    alpha = float('-inf')
    beta = float('inf')
